...
- Kết thúc bằng lời nhắn nhủ mang tinh thần Dịch học: con người có thể chuyển hóa vận mệnh bằng đức hạnh và hành động đúng thời ("Thiên hành kiện, quân tử dĩ tự cường bất tức").`;
      ...
      const luanGiaiContext = buildLuanGiaiChartContext(bestChartData, personName, chartData);
...
<scratchpad>
Hoàn thành TƯ DUY NHÁP bên dưới TRƯỚC khi viết luận giải. Phần scratchpad này là quá trình suy luận nội bộ — KHÔNG xuất ra cho người đọc.
...
Sau khi hoàn thành tư duy nháp, hãy viết bài luận giải hoàn chỉnh theo đúng cấu trúc sau. CHỈ xuất ra phần luận giải bên dưới, KHÔNG xuất ra phần scratchpad:
...
- Kết thúc bằng một lời chúc/nhắn nhủ mang tính an ủi, động viên, phù hợp với vận quẻ.`;
      ...
      const bestChartData = fullChartData || chartData;
      const vanHanContext = buildLuanGiaiChartContext(bestChartData, chartData?.personName, chartData);
...
## QUY TRÌNH SUY LUẬN VẬN HẠN

<scratchpad>
Hoàn thành TƯ DUY NHÁP bên dưới TRƯỚC khi viết luận giải. Phần scratchpad này là quá trình suy luận nội bộ — KHÔNG xuất ra cho người đọc.

**BƯỚC 1: Xác định các tầng thời gian đang hoạt động**

a) **Đại Hạn hiện tại:**
- Dựa trên ngày sinh và kỳ hạn được yêu cầu (${timeContext}), đương sự bao nhiêu tuổi?
- Đang ở Đại Hạn nào? (Cung nào? Can cung Đại Hạn là gì?)
- Sao nào tọa thủ tại cung Đại Hạn? Brightness?
- Trường Sinh tại cung Đại Hạn?

b) **Đại Hạn Tứ Hóa:**
- Can cung Đại Hạn → phi Lộc/Quyền/Khoa/Kỵ vào cung nào?
- Đại Hạn Kỵ phi đâu? Có trùng với Sinh Niên Kỵ không? (→ Điệp Kỵ)
- Đại Hạn Lộc phi đâu? Có trùng với Sinh Niên Lộc không? (→ Song Lộc)

c) **Lưu Niên Tứ Hóa (cho phân tích năm/tháng/tuần):**
- Can Chi của năm ${period?.split("/")[1] || period} → Lưu Niên Tứ Hóa là gì? Phi vào cung nào?
- Lưu Niên Mệnh Cung nằm ở đâu? (Xác định theo Địa Chi năm)
- Tương tác Lưu Niên Tứ Hóa × Sinh Niên Tứ Hóa × Đại Hạn Tứ Hóa?

${
  timeFrame === "month" || timeFrame === "week"
    ? `d) **Lưu Nguyệt Tứ Hóa (cho phân tích tháng/tuần):**
- Can Chi tháng ${period?.split("/")[0] || ""} → Lưu Nguyệt Tứ Hóa phi vào cung nào?
- Lưu Nguyệt Kỵ có trùng Lưu Niên Kỵ hoặc Sinh Niên Kỵ không?`
    : ""
}

**BƯỚC 2: Phát hiện Trùng Điệp Hóa (Critical)**

Kiểm tra TỪNG cung: có bao nhiêu tầng Hóa tinh cùng phi vào?
- Cung nào nhận Song Lộc hoặc Tam Lộc? → Điểm PHÁT của kỳ hạn
- Cung nào nhận Điệp Kỵ (2+ Kỵ)? → Điểm NGUY của kỳ hạn
- Cung nào có Lộc-Kỵ giao hội? → Điểm phức tạp, được-mất lẫn lộn
- Có Hóa Kỵ nhập Mộ ở tầng nào không?

**BƯỚC 3: Đánh giá Tam Hợp cho kỳ hạn**

- Tam Phương Tứ Chính của Lưu Niên/Nguyệt Mệnh Cung: sao nào hội chiếu?
- Cung Quan Lộc, Tài Bạch, Phu Thê trong kỳ hạn: sao lưu vào có cát hay hung?
- Trường Sinh tại cung Đại Hạn/Lưu Niên: năng lượng đang Vượng, Suy, hay Tuyệt?

**BƯỚC 4: Tổng hợp theo lĩnh vực**

Với mỗi lĩnh vực (Sự nghiệp, Tài lộc, Tình cảm, Sức khỏe):
- Tam Hợp nói gì? (sao + brightness + cách cục trong kỳ)
- Phi Tinh nói gì? (Tứ Hóa đa tầng phi vào cung liên quan)
- Trùng Điệp Hóa nào ảnh hưởng?
- Kết luận + hóa giải nếu cần
</scratchpad>

---

Sau khi hoàn thành tư duy nháp, hãy viết bài luận giải vận hạn hoàn chỉnh. CHỈ xuất ra phần luận giải, KHÔNG xuất scratchpad.
