...
export default function TuViIztroPage() {
  ...
  const loadAnalysis = useCallback(async () => {
    ...
    // Check if we already have a cached result for this chart in chart_analyses
    ...
    // No valid cache — call Claude with STREAMING
    isAnalyzingRef.current = true;
    setIsAnalyzing(true);
    setAnalysisError(false);
    setCachedAnalysis(null); // Clear so streaming text shows in UI
    ...
    try {
      ...
      // Save result to chart_analyses (upsert)
      if (existing) {
  ...
  const handleInterpret = useCallback(async () => {
    ...
    await loadAnalysis();
  }, [hasAccess, remaining, loadAnalysis]);
      ...
      <div className="max-w-5xl mx-auto space-y-6 p-4">
            ...
            <CardContent className="flex items-center gap-3 py-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400">
                  Vui lòng chờ trong giây lát, giao dịch đang được xử lý trên blockchain.
                </p>
              </div>
            </CardContent>
          </Card>
        ...
        {chart && (
          <div className="space-y-6">
            ...
            {/* ============================================================ */}
            {/* Luận giải chi tiết — Streaming + Cached                      */}
            {/* ============================================================ */}
            {(isAnalyzing || isStreamingAI) && !cachedAnalysis ? (
              /* ── STREAMING STATE: Text đang chảy real-time ── */
            ...
            ) : analysisError && !cachedAnalysis && !streamedText ? (
              /* ── ERROR STATE ── */
            ...
            ) : cachedAnalysis || streamedText ? (
              /* ── COMPLETED STATE: Full result ── */
              <div id="analysis-result" className="space-y-6">
                <ChartInterpretationDisplay chart={chart} />
                <Card className="p-6 bg-gradient-to-br from-surface-3 to-surface-2 border border-primary/20">
                  <h2 className="text-xl font-bold text-primary mb-4 flex items-center gap-2">
                  ...
                  <div className="space-y-1">{renderAnalysisMarkdown(cachedAnalysis || streamedText)}</div>
